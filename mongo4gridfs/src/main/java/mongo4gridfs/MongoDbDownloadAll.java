...
public class MongoDbDownloadAll {	// MongoDB 자료 전체 읽기
	public static void main(String[] args) {
		...
		try(MongoClient mongoClient = MongoClients.create(connectionString)) {
			...
			while(cursor.hasNext()) {
				...
				ObjectId field = gridFSFile.getObjectId();	// 파일의 ObjectId를 추출
				...
				try {
					...
					if(fileContent.trim().startsWith("[")) {
						...
						for(int i=0;i<jsonArray.length();i++) {
							JSONObject jsonObject = jsonArray.getJSONObject(i);		// 각 요소 데이터를 JSON으로 변환
							String req = jsonObject.getString("req");
							String res = jsonObject.getString("res");
							System.out.println("질문: " + req );
					...
					System.out.println("파일 찾기 실패: ObjectId => " + field.toHexString());
				}
			}
