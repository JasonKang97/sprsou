...
public class MongoDbDownload {	// MongoDB 자료 일부 읽기
	public static void main(String[] args) {
		...
		try(MongoClient mongoClient = MongoClients.create(connectionString)) {
			...
			// ObjectId 배열
			ObjectId[] fields = {
				new ObjectId("68673565994b820c30ce8b57"),
				new ObjectId("68673565994b820c30ce8b59"),
				new ObjectId("68673565994b820c30ce8b5d")
			};
	...
	private static void downloadAndPrintFileContent(GridFSBucket gridFSBucket, ObjectId field) {
		try {
			...
			// JSON 데이터 파싱
			JSONObject jsonObject = new JSONObject(fileContent);	// JSON 객체로 변환
