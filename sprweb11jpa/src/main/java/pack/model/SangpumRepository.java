...
// JpaRepository<Entity명, Entity의 @Id 필드타입>
public interface SangpumRepository extends JpaRepository<SangpumDto, Integer>{
	// Repository <- CrudRepository <- PagingAngSortingRePository <- JpaRepository
	...
	// select * from sangdata where sang like '%검색어%'에 의해 검색된 결과가 List 컬랙션에 담겨 반환됨.
	...
	// JPQL(Java Persistence Query Language) 사용 - JPA 지원 메소드로 처리할 수 없는 경우 사용하면 효과적이다.
