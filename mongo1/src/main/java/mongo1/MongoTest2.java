		...
		try(MongoClient mongoClient = MongoClients.create(uri)) {
			...
			for(Document doc:collection.find()) {
				...
				if(ageObj instanceof Integer) {
					...
					try {
						age = Integer.parseInt((String)ageObj);		// "22" => 22
...
//			collection.insertOne(newDocument);
			...
			// 특정자료 찾기(이름이 '신기해'인 자료 검색 후 수정 또는 삭제 수행)
			//자료 수정
			Document foundCustomer = collection.find(eq("name", "신기해")).first();
			
			if(foundCustomer != null) {
				ObjectId objectId = foundCustomer.getObjectId("_id");	// 검색된 document의 _id 얻기
				
//				collection.updateOne(eq("_id", objectId), Updates.combine(Updates.set("age", 44), Updates.set("gender", "여성")));
