...
public class AuthController {
	...
	public String login(@RequestParam(name="sabun")String sabun, @RequestParam(name="irum")String irum, Model model, HttpServletResponse response) {
		try {
			...
			jwtCookie.setSecure(false);		// https로만 사용할 경우에는 true로, http와 https 둘다는 false
			jwtCookie.setPath("/");
			jwtCookie.setMaxAge(3600);
			response.addCookie(jwtCookie);
			
			model.addAttribute("usrename", authentication.getName());
	...
	public String gugu(@RequestParam(name="num")int num, Model model, Authentication authentication) {
